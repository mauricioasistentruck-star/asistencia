    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-4">
      ...
      {viewMode === 'live' && (
        <>
                ...
                <div className="md:col-span-4 flex flex-wrap items-center gap-2 justify-end">
                  <div className="flex items-center gap-1">
                    <button
                      ...
                      className={'px-2.5 py-1.5 rounded-xl text-xs font-black transition-all cursor-pointer ' + (selectedDate === new Date().toISOString().split('T')[0] ? 'bg-orange-500 text-black shadow-md shadow-orange-500/20' : (isDark ? 'bg-zinc-900 text-zinc-400 border border-zinc-800 hover:text-white' : 'bg-orange-50 text-zinc-700 border border-orange-200 hover:text-black'))}
                    >
                      Hoy
                    </button>
                    <button
                      ...
                      className={'px-2.5 py-1.5 rounded-xl text-xs font-black transition-all cursor-pointer ' + (isDark ? 'bg-zinc-900 text-zinc-400 border border-zinc-800 hover:text-white' : 'bg-orange-50 text-zinc-700 border border-orange-200 hover:text-black')}
                    >
                      Ayer
                    </button>
                  </div>
          ...
          <div className={'border rounded-3xl p-3 shadow-2xl relative overflow-hidden flex flex-col ' + (isDark ? 'bg-zinc-950 border-zinc-800' : 'bg-white border-orange-200')}>
            
            {/* Barra Flotante de Telemetría sobre el Mapa */}
            <div className="absolute top-6 left-6 z-[400] flex flex-wrap gap-2 pointer-events-none">
              {!selectedUser && !selectedSavedRoute ? (
